...
Tile-based map generator that generates an abstract graph layout:
...
If none of the points are valid, the AI character uses another EQS query in a cone shape in front of it to move forward
...
- Chaining projectiles ability bouncing on enemies
...
I focused on keeping the code simple and readable. I developed system as separate projects so they could be independent which makes the architecture easier to understand and debug. Each system has data assets for configuration making it easier to customize.
